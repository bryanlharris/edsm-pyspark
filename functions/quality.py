...
def apply_dqx_checks(df: Any, settings: dict, spark: Any) -> Tuple[Any, Any]:
    ...
    from databricks.labs.dqx.check_funcs import DQXCheckRegistry

    # Register custom check functions generated by profiling notebooks. These
    # helpers mirror their builtin counterparts and are only registered when
    # ``apply_dqx_checks`` executes so that importing this module does not
    # require ``pyspark``.

    DQXCheckRegistry.register("min_max", min_max)
    DQXCheckRegistry.register("is_in", is_in)
    DQXCheckRegistry.register("is_not_null_or_empty", is_not_null_or_empty)
    DQXCheckRegistry.register("max_length", max_length)
    DQXCheckRegistry.register("matches_regex_list", matches_regex_list)
    DQXCheckRegistry.register("is_nonzero", is_nonzero)
    DQXCheckRegistry.register("starts_with_prefixes", starts_with_prefixes)
